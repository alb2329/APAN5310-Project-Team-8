...
# Convert table to 1NF, separate combined column values into multiple rows
dat <- dat %>%
...
dat$cast <- str_trim(dat$cast, side = "both")
dat$director <- str_trim(dat$director, side = "both")
dat$country <- str_trim(dat$country, side = "both")
dat$listed_in <- str_trim(dat$listed_in, side = "both")
